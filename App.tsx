...
function AppContent() {
    ...
    const initConnection = async () => {
        ...
        if (currentUser || isGuestMode) {
            if (currentUser) {
                ...
                // Make sure we have the streak even if reward not available
                setCurrentStreak(progressService.getProgress().dailyStreak || 0);
  ...
  useEffect(() => {
    ...
    if (activeThemeItem && activeThemeItem.colors) {
        ...
        document.body.style.setProperty('--color-brand-x', '#22d3ee'); 
        document.body.style.setProperty('--color-brand-o', '#f472b6'); 
        document.body.style.removeProperty('--app-bg-gradient');
    }
    ...
    <AppContext.Provider value={contextValue}>
      ...
      >
        ...
        <div className="flex-1 w-full relative z-10 overflow-y-auto overflow-x-hidden custom-scrollbar flex flex-col">
            <div className={`flex-1 flex flex-col items-center justify-start p-4 ${currentUser ? 'pt-28' : 'pt-4'} pb-4 min-h-full`}>
        ...
        <AnimatePresence>
            ...
            {showDailyReward && !resetToken && !verifyToken && (
                <DailyRewardModal onClose={() => setShowDailyReward(false)} currentStreak={currentStreak} />
            )}
