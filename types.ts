...
export interface User {
  ...
  questData?: {
      ...
      lastWinAt?: string; // ISO Date String
      equippedFrame?: string; // Added Frame
      lastPasswordChange?: string; // ISO Date String for Security Score
  };
  friendCode?: string;
...
export interface Notification {
    id: string;
    userId?: string;
    type: 'chat' | 'friend_request' | 'match_result' | 'system' | 'gift';
    title: string;
    message: string;
    timestamp: number;
    read: boolean;
...
export interface Room {
    ...
    doubleDown?: { offering: PlayerRole };
}
...
export interface UserProgress {
    ...
    dailyShop?: string[]; // Array of item IDs on sale
