...
import { checkWinner, calculateElo, calculateDrawElo, checkBadges, countThreats, MASTERY_CHALLENGES, calculateLevelProgress } from './gameLogic';
    ...
    io.on('connection', async (socket) => {
        ...
        if (!isGuest) {
            try {
                ...
                if (!user) {
                    // Invalid user ID from token, disconnect.
                    socket.disconnect();
                    return;
                }
            } catch (dbError) {
        ...
        socket.on('makeMove', async ({ roomId, index }, callback) => {
            ...
            if (!room || room.status !== 'playing' || room.isPaused) {
                // If room exists but state is invalid (e.g. already finished), sync client
            ...
            if (result.winner) {
                ...
                // CRITICAL FIX: Immediate broadcast so clients see "Game Over" instantly
                broadcastRoomUpdate(io, roomId);
                
                // Run DB ops in background, then broadcast again with XP/ELO data
                handleGameEnd(room).then(() => {
                    broadcastRoomUpdate(io, roomId);
                }).catch(err => console.error("Game End Error", err));
            } else {
        ...
        socket.on('sendInvite', async (friendId, roomId) => {
            ...
            if (dbMessage) {
                // Emit to both parties to update chat history immediately
        ...
        socket.on('sendDirectMessage', async ({ toUserId, text, replyTo, replayData, stickerId }) => {
          ...
          if (dbMessage) {
            ...
            // Emit to sender (so their UI also updates)
            socketService.emitToUser(userId, 'directMessage', dbMessage);
...
async function handleGameEnd(room: Room) {
    ...
    const dbUserX = await prisma.user.findUnique({ where: { id: pX.user.id }, select: { level: true, xp: true } });
    const dbUserO = await prisma.user.findUnique({ where: { id: pO.user.id }, select: { level: true, xp: true } });
    ...
    try {
        ...
        await notificationService.send(
                    ...
                    : `You lost to ${pO.user.displayName}. Lost ${ante} coins.`),
            {
                ...
                coinChange: pXNet,
                sender: { id: pO.user.id, displayName: pO.user.displayName, avatar: pO.user.avatar }
            }
        );
        ...
        await notificationService.send(
                    ...
                    : `You lost to ${pX.user.displayName}. Lost ${ante} coins.`),
            {
                ...
                coinChange: pONet,
                sender: { id: pX.user.id, displayName: pX.user.displayName, avatar: pX.user.avatar }
            }
        );
