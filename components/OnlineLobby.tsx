                    ...
                    <div className="flex-1 space-y-6">
                        ...
                        <div className="space-y-4">
                            {LOBBIES.map(lobby => {
                                ...
                                return (
                                    <Tooltip text={!canAfford ? `You need ${lobby.ante} coins` : `Ante: ${lobby.ante} Coins`} key={lobby.tier}>
                                        <button
                                        ...
                                        >
                                            ...
                                            <div className="relative z-10 text-left">
                                                ...
                                                <div className="text-xs font-bold text-yellow-300 flex items-center gap-1 mt-1">
                                                    <CoinIcon className="w-3 h-3"/> PRIZE POT: {lobby.ante * 2}+
                                                </div>
                                            </div>
                                            <div className="relative z-10 flex flex-col items-center justify-center p-3 rounded-full bg-black/30 border border-white/10 shadow-inner">
                                                <SwordIcon className="w-6 h-6 text-white"/>
                                                <span className="text-[10px] font-bold mt-1">{isCreating === lobby.tier ? '...' : 'DUEL'}</span>
                                            </div>
                                        </button>
                                    </Tooltip>
                                );
                        ...
                        <div className="flex flex-col gap-3">
                            <div className="relative">
                                <input 
                                    ...
                                    placeholder="CODE"
                                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-5 font-mono text-center text-4xl font-bold tracking-[0.5em] focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:bg-white/10 placeholder:text-white/10 uppercase"
                                    maxLength={6}
                                />
                                <button 
                                    onClick={handlePaste}
                    ...
                    <div className="space-y-2">
                        {currentList.length === 0 ? (
                            ...
                            currentList.map((user, idx) => {
                                ...
                                return (
                                    <div key={user.id} className={`flex items-center gap-4 p-3 rounded-xl border ${isMe ? 'bg-cyan-500/10 border-cyan-500/30' : 'bg-white/5 border-white/5'}`}>
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2">
                                                <p className="font-bold text-sm truncate">{user.displayName} {isMe && '(You)'}</p>
