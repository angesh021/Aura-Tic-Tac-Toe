...
const Header: React.FC<HeaderProps> = ({ onGoToProfile, onGoToHistory, onOpenSettings }) => {
    ...
    // Detect if we are waiting for initial coin hydration to prevent "+700" popup on refresh
    // If app coins is 0 but user profile has coins, we assume the next update is just hydration.
    const isHydratingRef = useRef((app?.coins === 0 && (currentUser?.coins || 0) > 0));

    // Subscribe to game store to auto-close menus when game starts/updates
    ...
    useEffect(() => {
        if (currentUser?.xp !== undefined) {
            ...
            if (currentUser.level > (prevLevelRef.current || 1)) {
                // rough estimate for visual pop on level up
    ...
    useEffect(() => {
        if (app?.coins !== undefined) {
            ...
            if (diff !== 0) {
                // If we flagged this as a hydration step, skip the animation
    ...
    return (
        <>
            <header className="fixed top-0 left-0 right-0 z-40 h-24 px-4 flex justify-center items-center pointer-events-none">
                <motion.div 
                    className="w-full max-w-7xl h-16 px-4 flex items-center justify-between glass-panel rounded-2xl pointer-events-auto shadow-md dark:shadow-none"
                    initial={{ y: -100, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
                    transition={{ duration: 0.5, ease: 'easeOut' }}
                >
                    {/* Left: Logo */}
                    <button onClick={app?.goHome} className="text-2xl font-bold tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-cyan-600 to-pink-600 dark:from-cyan-400 dark:to-pink-500 hover:opacity-80 transition-opacity">
                        Aura
                    </button>
                    ...
                    <div className="hidden md:flex items-center gap-2">
                        ...
                        <Tooltip text="Aura Coins">
                            ...
                            >
                                ...
                                <span className={`text-sm font-bold block ${coinChange > 0 ? 'text-green-700 dark:text-green-300' : (coinChange < 0 ? 'text-red-700 dark:text-red-300' : 'text-gray-900 dark:text-white')}`}>{app?.coins}</span>
                                <AnimatePresence>
                                    {coinChange !== 0 && (
                                        <motion.span 
                ...
                {isMobileMenuOpen && (
                    ...
                    >
                        <motion.div
                            ...
                            className="absolute top-0 right-0 h-full w-full max-w-sm bg-white dark:bg-[#0f172a]/95 border-l border-gray-200 dark:border-white/10 flex flex-col p-6 shadow-2xl"
                        >
                           ...
                           <div className="flex gap-4 mb-8">
                                <div className="flex-1 flex items-center gap-3 p-3 rounded-xl bg-gray-100 dark:bg-black/20">
                                ...
                                <div className="flex-1 flex flex-col justify-center p-3 rounded-xl bg-gray-100 dark:bg-black/20">
