...
import { MessageIcon, SmileIcon, CloseIcon, SendIcon, LightningIcon, ReplyIcon } from './Icons';
...
const GameChat: React.FC<GameChatProps> = ({ 
    ...
    const handleQuickChat = (text: string) => {
        // FIX: `onSendMessage` expects a second argument for `replyTo`.
        onSendMessage(text);
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className={`p-3 rounded-2xl text-sm max-w-[80%] ${isMe ? 'bg-cyan-600 text-white rounded-br-md' : 'bg-gray-700 text-gray-200 rounded-bl-md'}`}>
                                        {!isMe && <div className="text-[10px] font-bold text-cyan-400 mb-1">{msg.senderName}</div>}
