...
import { OIcon, XIcon, ObstacleIcon, CrownIcon } from './Icons';
...
const Square: React.FC<SquareProps> = ({ value, onClick, isWinner, isHinted, boardSize, cursor, skin = 'skin-classic', isSummary = false }) => {
  ...
  const renderPiece = (type: 'X' | 'O') => {
      if (skin === 'skin-golden') {
          return (
              <div className="relative w-full h-full p-1">
                  <div className="absolute inset-0 bg-gradient-to-br from-yellow-300 to-amber-500 opacity-20 blur-md"></div>
                  {type === 'X' 
                    ? <XIcon className="w-full h-full text-yellow-400 filter drop-shadow-[0_0_8px_#facc15]" /> 
                    : <OIcon className="w-full h-full text-yellow-400 filter drop-shadow-[0_0_8px_#facc15]" />}
              </div>
          );
      }
      if (skin === 'skin-grandmaster') {
          return (
              <div className="relative w-full h-full p-1">
                  <div className="absolute inset-0 bg-gradient-to-br from-purple-500 to-indigo-600 opacity-20 blur-md"></div>
                  {type === 'X' 
                    ? <XIcon className="w-full h-full text-purple-300 filter drop-shadow-[0_0_10px_#c084fc]" /> 
                    : <OIcon className="w-full h-full text-purple-300 filter drop-shadow-[0_0_10px_#c084fc]" />}
                  <CrownIcon className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1/3 h-1/3 text-yellow-400 opacity-80" fill={true}/>
              </div>
          );
      }
      if (skin === 'skin-emoji') {
      ...
      if (skin === 'skin-neon') {
          if (type === 'X') {
              return (
                  <svg viewBox="0 0 100 100" className="w-full h-full filter drop-shadow-[0_0_8px_var(--color-brand-x)]">
          ...
          return (
              <svg viewBox="0 0 100 100" className="w-full h-full filter drop-shadow-[0_0_8px_var(--color-brand-o)]">
