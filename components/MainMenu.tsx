...
import { CloseIcon, OIcon, XIcon, GridIcon, UserIcon, MapIcon, ShopIcon, QuestIcon, SwordIcon } from './Icons';
...
const MainMenu: React.FC<MainMenuProps> = ({ onStartGame, onGoToOnlineLobby, onGoToCampaign, onGoToShop }) => {
  ...
  const greeting = auth?.currentUser?.displayName ? getPersonalizedGreeting(auth.currentUser.displayName) : 'Welcome';

  return (
    <>
    <motion.div 
      className="w-full max-w-6xl flex flex-col lg:flex-row gap-4 h-auto lg:h-[600px] px-2 md:px-0 pb-4 lg:pb-0"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    >
        {/* Left Side: Identity & Modes */}
        <div className="flex-1 glass-panel rounded-3xl p-5 flex flex-col relative overflow-hidden min-h-[350px] lg:min-h-0">
             ...
             <div className="flex justify-between items-start mb-4">
                 <div>
                    ...
                    <p className="text-cyan-600 dark:text-cyan-400 font-medium text-sm md:text-sm">{greeting}</p>
                 </div>
                 ...
                 <div className="flex gap-2">
                     ...
                     <motion.button 
                        onClick={onGoToShop}
                        whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                        className="p-2 rounded-xl bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 hover:bg-yellow-500/20 flex flex-col items-center gap-0.5 min-w-[50px]"
                     >
             ...
             {/* Game Modes Grid - Compacted */}
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 flex-1 overflow-y-auto custom-scrollbar pr-1 pt-1">
        ...
        {/* Right Side: Settings - Compacted */}
        <div className="w-full lg:w-[420px] glass-panel rounded-3xl p-5 flex flex-col h-[400px] lg:h-auto">
    ...
    <AnimatePresence>
        {showQuestBoard && (
            <Modal onClose={() => setShowQuestBoard(false)}>
                    ...
                    <div className="flex flex-col md:flex-row gap-4 items-center justify-center relative mb-8">
                        ...
                        <div className="flex-1 w-full bg-cyan-50/50 dark:bg-cyan-950/30 border border-cyan-200 dark:border-cyan-500/30 rounded-2xl p-4 flex flex-col items-center gap-3 relative overflow-hidden group">
                             ...
                             <div className="w-20 h-20 rounded-full border-2 border-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.3)] bg-gray-100 dark:bg-black overflow-hidden">
                                 <UserAvatar avatarId="avatar-1" className="w-full h-full" />
                             </div>
