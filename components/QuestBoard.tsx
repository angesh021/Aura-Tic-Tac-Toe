...
const QuestCard: React.FC<{
    ...
    return (
        <motion.div
            ...
            className={`
                relative rounded-[24px] shadow-xl group
            ...
            `}
        >
            {/* Separate Border Layer to fix clipping artifacts on rounded corners */}
            <div className={`absolute inset-0 rounded-[24px] border pointer-events-none z-20 ${isRare ? 'border-yellow-500/30' : 'border-white/10'} ${quest.completed && !quest.claimed ? 'ring-2 ring-yellow-400/50' : ''}`}></div>

            <div className="absolute inset-0 rounded-[24px] overflow-hidden transform-gpu">
                {/* Background & Effects */}
                ...
                {/* Holographic Shine for Rare Quests */}
                {isRare && (
                    <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/5 to-transparent opacity-30 pointer-events-none group-hover:opacity-50 transition-opacity" />
                )}
                
                {/* Ambient Glow - Using specific classes from config to avoid dynamic class purge */}
                <div className={`absolute -top-20 -right-20 w-64 h-64 rounded-full blur-3xl transition-opacity duration-500 opacity-60 group-hover:opacity-80 ${config.glow}`} />
            </div>
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex justify-between mt-1">
                                    <div className="text-sm text-gray-400 flex items-center gap-1">
                                        Reward
                                        {multiplier > 1 && <span className="text-[10px] bg-white/10 px-1 rounded text-gray-300">({quest.reward} x {multiplier})</span>}
                                    </div>
