...
const SocialHub: React.FC<SocialHubProps> = ({ onClose, initialTargetId }) => {
    ...
    const filteredFriends = useMemo(() => {
        ...
        // Sort: Online > In Game > Offline, then Alphabetical
    ...
    }, [friends, friendStatuses, searchQuery, auth?.currentUser?.id]);
                                        ...
                                        <div key={req.id} className="p-3 mx-1 rounded-xl bg-gradient-to-r from-orange-900/20 to-transparent border border-orange-500/20 flex flex-col gap-2">
                                            <div className="flex items-center gap-3">
                                                <div className="w-8 h-8 rounded-full overflow-hidden bg-black border border-white/10 shrink-0">
                                                    <UserAvatar avatarId={req.sender?.avatar || 'avatar-1'} className="w-full h-full" />
                                                </div>
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 text-left min-w-0">
                                                ...
                                                <div className="text-[10px] text-gray-500 truncate font-medium">
                                                    {status === 'ONLINE' ? <span className="text-green-400">Online</span> : 
                                                     status === 'IN_GAME' ? <span className="text-orange-400">In Match</span> : 
                                                     status === 'WAITING' ? <span className="text-blue-400">In Lobby</span> : 'Offline'}
                                                </div>
                                            </div>
                                        </button>
                                    );
