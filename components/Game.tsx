...
import { CloseIcon, CopyIcon, GridIcon, TrophyIcon, ObstacleIcon, SkullIcon, CheckIcon, ClockIcon, EyeIcon, LightningIcon, LinkIcon, UndoIcon, HintIcon, BombIcon, ShieldIcon, LogoutIcon, MapIcon, DoubleIcon, ConvertIcon, XIcon, OIcon, PauseIcon, CoinIcon } from './Icons';
...
const PlayerInfoPanel = ({ seat, fallbackName, role, isActive, label, avatarId, elo, currentEmote }: { seat?: PlayerSeat, fallbackName: string, role: Player, isActive: boolean, label: string, avatarId: string, elo?: number, currentEmote?: string }) => {
        ...
        `}>
            ...
            <div className="relative mb-3">
                <div className={`w-16 h-16 md:w-20 md:h-20 rounded-full p-1 border-2 ${role === Player.X ? 'border-cyan-500' : 'border-pink-500'} overflow-hidden`}>
                    <UserAvatar avatarId={avatarId} className="w-full h-full rounded-full" />
                </div>
                {!isConnected && (
...
const MobilePlayerInfo = ({ player, isActive, seat, fallbackName, avatarId, blitzTime, label, currentEmote }: { player: Player, isActive: boolean, seat?: PlayerSeat, fallbackName: string, avatarId: string, blitzTime?: string, label: string, currentEmote?: string }) => {
    let statusText = "PLAYING";
    if (label === "YOU") statusText = "YOUR TURN";
    else if (label === "AURA AI" || label === "BOSS") statusText = "THINKING...";
            ...
            <div className="relative">
                ...
                <AnimatePresence>
                    {currentEmote && (
                        <motion.div 
                            initial={{ opacity: 0, y: 0, scale: 0.5 }}
                            animate={{ opacity: 1, y: 30, scale: 2 }} // Float downwards on mobile to be visible over board
...
interface GameProps {
  ...
  onlineRoom?: Room | null;
  playerNames?: { [key in Player]?: string };
  campaignLevel?: CampaignLevel;
  onNextLevel?: (level: CampaignLevel) => void;
}

const Game: React.FC<GameProps> = ({ userId, gameMode, gameSettings, onlineRoom = null, playerNames, campaignLevel, onNextLevel }) => {
  const isOnline = gameMode === GameMode.ONLINE && !!onlineRoom;
  const toast = useToast();
  ...
  const TURN_DURATION = gameSettings.turnDuration || 30; // Use setting or default

  // Cache for online players to prevent "Waiting..." on disconnect/leave
  ...
  const getInitialPowerUps = useCallback(() => {
      // If disabled in settings, return empty/false map
      if (gameSettings.powerUps === false) {
          return { [Player.X]: {}, [Player.O]: {} };
      }

      // If Local PvP, give both players 1 charge of everything (Sandbox mode)
      ...
      // Standard Logic: Check Inventory
      const inv = auth?.currentUser?.inventory || [];
      const isGuest = auth?.currentUser?.isGuest;
      const hasDestroy = inv.includes('powerup-destroy') || isGuest;
      const hasWall = inv.includes('powerup-wall') || isGuest;
      const hasDouble = inv.includes('powerup-double') || isGuest;
      const hasConvert = inv.includes('powerup-convert') || isGuest;
  ...
  }, [gameMode, auth?.currentUser, gameSettings.powerUps]);

  const [board, setBoard] = useState<BoardState>(isOnline ? onlineRoom.board : createInitialBoard());
  
  // Calculate starting player for local/AI games based on settings
  const [currentPlayer, setCurrentPlayer] = useState<Player>(() => {
      if (isOnline) return onlineRoom.currentPlayer;
  ...
  const [winner, setWinner] = useState<Player | 'draw' | null>(isOnline ? onlineRoom.winner : null);
  const [winningLine, setWinningLine] = useState<number[] | null>(isOnline ? onlineRoom.winningLine : null);
  const [winReason, setWinReason] = useState<'standard' | 'forfeit' | 'timeout' | 'disconnect' | undefined>(isOnline ? onlineRoom.winReason : undefined);
  const [isAiThinking, setIsAiThinking] = useState(false);
  const [moves, setMoves] = useState<Move[]>(isOnline ? onlineRoom.moves : []);
  const [initialBoard, setInitialBoard] = useState<BoardState>(isOnline ? onlineRoom.initialBoard : board);
  
  // Initialize PowerUps using helper
  ...
  const mySeat = useMemo(() => onlineRoom?.players.find(p => p.user.id === auth?.currentUser?.id), [onlineRoom, auth?.currentUser]);
  const isSpectator = !!(isOnline && (mySeat?.role === 'spectator' || auth?.currentUser?.isGuest));
  const isMyTurnOnline = isOnline && mySeat?.role === onlineRoom.currentPlayer && !onlineRoom.winner && onlineRoom.status === 'playing';
  ...
  const opponentSeat = useMemo(() => {
      ...
      return onlineRoom?.players.find(p => p.role !== 'spectator' && p.user.id !== auth?.currentUser?.id);
  }, [isOnline, onlineRoom, auth?.currentUser?.id]);
  ...
  const getPlayerLabel = useCallback((targetRole: Player) => {
      if (isOnline && onlineRoom) {
           ...
           // If seat missing, check participants
  ...
  useEffect(() => {
    if (isOnline) {
      ...
      // Synchronize timer: If running and not paused
  ...
  }, [isOnline, gameSettings.blitzMode, winner, currentPlayer, onlineRoom?.status, onlineRoom?.isPaused]);
  ...
  const handleGameOver = useCallback(async (newWinner: Player | 'draw', reason: 'standard' | 'forfeit' | 'timeout' | 'disconnect' = 'standard') => {
    ...
    if (gameMode === GameMode.CAMPAIGN && campaignLevel && newWinner === Player.X) {
    ...
    if (gameMode === GameMode.AI && newWinner === Player.O) {
        // Fallback or explicit taunt if needed, but Gemini handles during move
    }
  ...
  const resetGame = useCallback(() => {
    ...
    // Use the helper to reset powerups correctly based on mode
  ...
  const handleRematchRequest = () => {
     if (isOnline && !isSpectator) onlineService.requestRematch(onlineRoom.id);
  }
  ...
  useEffect(() => {
    ...
    useEffect(() => {
        ...
        const performAiMove = () => {
            ...
            setTimeout(async () => {
                 const bestMoveAnalysis = findBestMove(board, gameSettings, usedTaunts);
                 const now = Date.now();
                 
                 // --- GEMINI INTEGRATION ---
                 // If enough time passed, fetch dynamic taunt
                 if (now - lastTauntTimeRef.current > 8000) {
                    // Start fetching taunt asynchronously (fire and forget)
   ...
   useEffect(() => {
      ...
      setTurnTimer(t => {
        if (t <= 0) {
            ...
            if (!isOnline) {
                ...
                // Attempt to claim timeout victory on server
                onlineService.claimTimeout(onlineRoom.id);
            }
            return 0;
        }
        if (t <= 5) playSound('timerTick');
        return t - 1;
      });
    }, 1000);
    return () => clearInterval(interval);
    ...
    if (winner) {
        ...
        if (isOnline && !isSpectator) {
            ...
            // Assuming myRole is Player.X or Player.O here as !isSpectator
            if(xRole){
        ...
        return (
            <GameSummary
                ...
                // Pass pot and ante if available
                pot={onlineRoom?.pot}
    ...
    if (isOnline && ['waiting', 'ready', 'confirming_wager'].includes(onlineRoom.status) && !onlineRoom.winner && !isPaused) {
        if (onlineRoom.status === 'confirming_wager') {
            return <WagerConfirmation
                room={onlineRoom}
                currentUserId={userId}
                onConfirm={() => onlineService.confirmWager(onlineRoom.id)}
    ...
    >
      ...
      <div className="md:hidden shrink-0 flex justify-between items-center px-4 py-2 bg-white/10 dark:bg-white/5 rounded-xl mx-2 mb-2 border border-white/10 relative z-10">
          <MobilePlayerInfo 
            ...
            seat={isOnline ? onlineRoom.players.find(p => p.role === Player.X) : undefined}
            ...
            currentEmote={getPlayerEmote(isOnline ? onlineRoom.players.find(p => p.role === Player.X)?.user.id : undefined)}
          />
          ...
          <div className="text-center px-2">
             ...
             {!gameSettings.blitzMode && !isOnline && (
                 <div className="text-[10px] font-mono text-cyan-400">{turnTimer}s</div>
             )}
          </div>
          ...
          <MobilePlayerInfo 
            ...
            seat={isOnline ? onlineRoom.players.find(p => p.role === Player.O) : undefined}
            ...
            currentEmote={getPlayerEmote(isOnline ? onlineRoom.players.find(p => p.role === Player.O)?.user.id : undefined)}
          />
      </div>

      {/* Mobile Action Status Prompt */}
      {activePowerUp && (
          <div className="md:hidden shrink-0 text-center mb-2 animate-pulse">
              <span className="text-xs font-bold text-white bg-red-500/80 px-3 py-1 rounded-full shadow-lg border border-red-400/50">
                  {activePowerUp === 'destroy' && "Tap opponent's piece to destroy!"}
                  {activePowerUp === 'wall' && "Tap empty square to place wall!"}
                  {activePowerUp === 'double' && "Double Strike Active!"}
                  {activePowerUp === 'convert' && "Tap opponent's piece to convert!"}
              </span>
          </div>
      )}
        ...
        <div className="flex flex-col items-center justify-center h-full w-full max-h-full">
            ...
            <div className="w-full h-full flex items-center justify-center p-2 overflow-hidden">
                <div className="w-full h-full flex items-center justify-center">
                    {/* Constrain Board Size to fit height/width */}
      ...
      {/* Bottom Controls Area: Power-Ups or Emoji Bar */}
      <div className="shrink-0 w-full flex justify-center pb-4 pt-2 z-20 safe-area-pb px-4">
          ...
          {isOnline && onlineRoom && !isSpectator && (
