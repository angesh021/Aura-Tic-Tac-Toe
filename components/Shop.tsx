...
import { CoinIcon, CheckCircleIcon, LockIcon, CloseIcon, UserIcon, PaletteIcon, ShapesIcon, XIcon, OIcon, LightningIcon, BombIcon, ShieldIcon, DoubleIcon, ConvertIcon } from './Icons';
...
type Tab = 'avatar' | 'theme' | 'skin' | 'powerup';
...
const Shop: React.FC<ShopProps> = ({ onClose }) => {
    ...
    const initiatePurchase = (item: any) => {
        ...
        setConfirmItem({ id: item.id, name: item.name, cost: item.cost });
    };
    ...
    const handleEquip = async (itemId: string, type: Tab) => {
        if (type === 'powerup') return; // Cannot equip powerups, they are passive unlocks

        const success = await progressService.equipItem(itemId, type);
        if (success) {
            if (type === 'avatar') auth?.updateUser({ avatar: itemId });
            if (type === 'theme' || type === 'skin') {
                app?.refreshUser();
            }
    ...
    return (
        <>
        <motion.div 
            className="w-full max-w-5xl p-6 bg-white/10 dark:bg-gray-900/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/10 h-full flex flex-col relative overflow-hidden"
        ...
        >
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 shrink-0 relative z-10 gap-4">
                <div>
                    <h1 className="text-4xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-pink-400 to-purple-600">
                        Marketplace
                    </h1>
                    <p className="text-gray-400 text-sm font-medium">Spend your hard-earned coins</p>
                </div>
                <div className="flex items-center gap-4 self-end md:self-auto">
                    <div className="px-4 py-2 bg-yellow-500/10 border border-yellow-500/30 rounded-xl flex items-center gap-2 shadow-lg shadow-yellow-500/5">
                        <CoinIcon className="w-5 h-5 text-yellow-400" />
                        <span className="font-bold text-yellow-100 text-lg">{progress.coins}</span>
                    </div>
                    <button onClick={onClose} className="p-2.5 rounded-full bg-white/5 hover:bg-white/10 border border-white/5 transition-colors"><CloseIcon className="w-6 h-6" /></button>
                </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-6 shrink-0 relative z-10">
                {/* Tabs */}
                <div className="flex-1 flex gap-1 p-1 bg-black/20 rounded-2xl border border-white/5 overflow-x-auto no-scrollbar">
                    {[
                        ...
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl font-bold text-sm transition-all duration-200 whitespace-nowrap
                                ${activeTab === tab.id ? 'bg-white/10 text-white shadow-md border border-white/10' : 'text-gray-500 hover:text-gray-300 hover:bg-white/5 border border-transparent'}
                            `}
                        >
                ...
                <button 
                    ...
                    className={`px-4 py-2 rounded-xl border font-bold text-sm transition-all whitespace-nowrap
                        ${filterOwned ? 'bg-green-500/20 border-green-500/30 text-green-400' : 'bg-white/5 border-white/10 text-gray-400 hover:bg-white/10'}
                    `}
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 overflow-y-auto custom-scrollbar p-1 flex-1 relative z-10 pb-4">
                {filteredItems.map((item, idx) => {
                    ...
                    const isEquipped = getEquippedId(activeTab) === item.assetId;
                        ...
                        <motion.div 
                            ...
                            className={`
                                ...
                                ${isEquipped 
                                    ...
                                    : 'bg-white/5 border-white/5 hover:bg-white/10 hover:border-white/10 hover:-translate-y-1 shadow-lg'
                                }
                            `}
                        >
                            <div className="mb-4 relative w-full flex justify-center min-h-[100px] items-center">
                                {renderPreview(item)}
                                {isEquipped && (
                                    <div className="absolute top-0 right-0 bg-green-500 text-black text-[10px] font-bold px-2 py-0.5 rounded-full shadow-lg flex items-center gap-1">
                                        <CheckCircleIcon className="w-3 h-3" /> EQUIPPED
                                    </div>
                                )}
                                {activeTab === 'powerup' && isOwned && (
                            ...
                            <div className="text-center w-full mb-4">
                                ...
                                {item.description && <p className="text-xs text-gray-500 mt-1 line-clamp-2 min-h-[2.5em] leading-relaxed">{item.description}</p>}
                            </div>

                            <div className="w-full mt-auto">
                                {isOwned ? (
                                    activeTab === 'powerup' ? (
                                        <button disabled className="w-full py-3 rounded-xl font-bold text-xs bg-white/5 border border-white/10 text-gray-400 cursor-default">
                                            Purchased
                                        </button>
                                    ) : (
                                        <button 
                                            onClick={() => handleEquip(item.assetId, activeTab)}
                                            disabled={isEquipped}
                                            className={`w-full py-3 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-all
                                                ${isEquipped 
                                                    ? 'bg-transparent text-green-500 border border-green-500/30 opacity-50 cursor-default' 
                                    ...
                                    <button 
                                        ...
                                        className={`w-full py-3 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-all border
                                            ${canAfford 
                                                ? 'bg-yellow-500/10 border-yellow-500/50 text-yellow-400 hover:bg-yellow-500 hover:text-black hover:border-yellow-500' 
                                                : 'bg-white/5 border-white/5 text-gray-500 cursor-not-allowed'
                                            }`}
                                    >
                                        {canAfford ? (
                                            <>Purchase <span className="bg-black/20 px-1.5 rounded text-[10px]">{item.cost}</span></>
                                        ) : (
                                            <><LockIcon className="w-3 h-3" /> {item.cost}</>
                                        )}
                                    </button>
                                )}
                            </div>
            ...
            {confirmItem && (
                <Modal onClose={() => !isProcessing && setConfirmItem(null)}>
                    <div className="text-center space-y-6 p-2">
                        <div className="w-20 h-20 mx-auto bg-yellow-500/20 rounded-full flex items-center justify-center border-2 border-yellow-500 shadow-[0_0_30px_rgba(234,179,8,0.3)]">
                            <CoinIcon className="w-10 h-10 text-yellow-400" />
                        </div>
                        <div>
                            <h2 className="text-2xl font-bold text-white mb-2">Confirm Purchase</h2>
                            <p className="text-gray-400">
                                Buy <strong className="text-white">{confirmItem.name}</strong> for <strong className="text-yellow-400">{confirmItem.cost}</strong> coins?
                            </p>
                        </div>
                        <div className="flex gap-3 pt-2">
                            ...
                            <button 
                                ...
                                disabled={isProcessing}
                                className="flex-1 py-3 rounded-xl font-bold text-black bg-yellow-500 hover:bg-yellow-400 transition-colors shadow-lg shadow-yellow-500/20 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                            >
                                {isProcessing ? (
                                    <>
