...
const ChecklistItem: React.FC<{ 
    ...
    actionLabel?: string,
    onAction?: () => void
}> = ({ label, points, completed, icon, delay, actionLabel, onAction }) => (
    <motion.div 
        initial={{ opacity: 0, x: 20 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ delay }}
        className={`flex items-center justify-between p-3 rounded-xl border mb-2 group transition-all duration-300
            ${completed 
    ...
    >
        <div className="flex items-center gap-3">
            <div className={`p-2 rounded-lg transition-colors ${completed ? 'bg-green-500/20 text-green-500' : 'bg-gray-700/50 text-gray-400 group-hover:text-white'}`}>
                {completed ? <CheckIcon className="w-4 h-4" /> : icon}
            </div>
            <div className="flex flex-col">
                <span className={`text-sm font-bold ${completed ? 'text-gray-300 line-through decoration-white/20' : 'text-gray-200'}`}>{label}</span>
                <span className={`text-[10px] font-mono font-bold ${completed ? 'text-green-500/70' : 'text-gray-500'}`}>
                    +{points} Points
                </span>
            </div>
        </div>
        ...
        {completed ? (
            ...
            onAction ? (
                <button 
                    onClick={onAction}
                    className="px-3 py-1.5 bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-bold rounded-lg transition-colors shadow-lg shadow-cyan-500/20"
                >
                    {actionLabel}
                </button>
            ) : (
